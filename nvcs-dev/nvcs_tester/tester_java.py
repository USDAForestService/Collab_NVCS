...
def run(outfile, debugfile, dbfile, plottbl, temp_path, invyrs, where):
    ...
    with Timer() as t:
        with open(outfile, mode='w', encoding='utf-8') as f:
            plots = json_query(dbfile, plottbl, full_where)
            for plot in plots:
                ident = plot[0]
                rows_json = plot[1]
                with open(temp_path, mode='w', encoding='utf-8') as temp:
                    temp.write(rows_json)
                string_path = java_classify(temp_path)
                path = string_path.split(", ")
                wrapped_path = f"[{string_path}]"
                if path[-1]:
                    f.write('{0}\t{1}\t{2}\n'.format(ident, path[-1], wrapped_path))
                else:
                    f.write('{0}\t{1}\t{2}\n'.format(ident, -1, wrapped_path))
    print('Run took %.03f sec.' % t.interval)

def json_query(dbfile, tbl, where):
    query = f"""
        SELECT
            IDENT,
...
def java_classify(json_path):
    java_classes = "C:/GitHub/FS-Enterprise/NVCS/nvcs_java/target/classes"
    args = ['java', '-cp', java_classes, 'nvcs.App', json_path]
    try:
