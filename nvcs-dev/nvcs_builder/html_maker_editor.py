...
def make_html(directory):
    prolog = ['<!DOCTYPE html>',
              ...
              'var appEdit = "\\"C:\\\\Program Files (x86)\\\\IDM Computer Solutions\\\\UltraEdit\\\\Uedit32.exe\\"";',
              'shell.Run(appEdit + " " + file);',
              '}',
              '</script>'
              '</head>',
              '<body>',
    ...
    body = ['{0}<input type="checkbox" name="dummy" value="0">{1}<a href="javascript:runProgram(\'{2}\')">{3}</a><br>'.format(index, level, ident, filename) for index, level, ident, filename in bodydata]
...
if __name__ == '__main__':
    import html_maker_editor
    html_maker_editor.write_html('C:/nvcs-workspace/nvcs_west/nvcs_builder/generated-code/html', 'C:/nvcs-workspace/nvcs_west/nvcs_builder/generated-code/html/key-editor.html')
    pass
