#!python3
import cx_Oracle
import key_eastern_us
from key_eastern_us import Plot
from key_eastern_us import Tree
...
def fetch_nims_nvcs_input(schema_name, con, cnd_cn):
    ...
    condcur = con.cursor()
    condcur.execute(
        "select cnd_cn, stateab, ecosubcd_actual, plantation, hydric, riverine from "
        + schema_name
        + ".nims_cond_nvcs_input_vw where cnd_cn=:p_cnd_cn",
        p_cnd_cn=cnd_cn,
    )
    for c in condcur:
        ...
        treecur = con.cursor()
        treecur.execute(
            "select species_name, riv, wetland, ruderal, exotic, softwoodhardwood, planted from "
            + schema_name
            + ".nims_species_nvcs_input_vw where cnd_cn = :p_cnd_cn",
            p_cnd_cn=cnd_cn,
        )
        trees = []
        for t in treecur:
...
def lookup_nvcs_hierarchy_struct_cn(schema_name, con, node_id, stdorgcd):
    ...
    if node_id == NID_ALGORITHM_FAILED or node_id == NID_ECOREGION_NOT_SUPPORTED:
        ...
        cur.execute(
            "select cn, 'HIGH' confidence from "
            + schema_name
            + ".ref_nvcs_hierarchy_strct where primary_class=:p_primary_class and hierarchy_level_label in ('MACROGROUP', 'SUBTYPE') and nvcs_code=:p_node_id",
            p_primary_class=primary_class,
            p_node_id=str(node_id),
        )
        for rec in cur:
            (rnvcshs_cn, confidence) = (rec[0], rec[1])
    else:
        cur.execute(
            "select rnvcshs_cn, NVL(confidence, 'HIGH') confidence from "
            + schema_name
            + ".ref_nvcs_algorithm_node where node_id=:p_node_id",
            p_node_id=node_id,
        )
        for rec in cur:
            (rnvcshs_cn, confidence) = (rec[0], rec[1])
...
def update_nims_cond_nvcs_tbl(schema_name, con, cnd_cn, node_id, stdorgcd):
    cur = con.cursor()
    (rnvcshs_cn, confidence) = lookup_nvcs_hierarchy_struct_cn(
        schema_name, con, node_id, stdorgcd
    )
    if rnvcshs_cn is None:
        (rnvcshs_cn, confidence) = lookup_nvcs_hierarchy_struct_cn(
            schema_name, con, NID_ALGORITHM_FAILED, stdorgcd
        )
    # print("cnd_cn=%s, rnvcshs_cn=%s, confidence=%s" % (cnd_cn, rnvcshs_cn, confidence))
    cur.execute(
        "delete from " + schema_name + ".nims_cond_nvcs_tbl where cnd_cn=:p_cnd_cn",
        p_cnd_cn=cnd_cn,
    )
    cur.execute(
        "insert into "
        + schema_name
        + ".nims_cond_nvcs_tbl (cnd_cn, rnvcshs_cn, confidence, publish) select :p_cnd_cn, cn, :p_confidence, 'Y' from "
        + schema_name
        + ".ref_nvcs_hierarchy_strct start with cn = :p_rnvcshs_cn connect by prior parent_cn = cn",
        p_cnd_cn=cnd_cn,
        p_rnvcshs_cn=rnvcshs_cn,
...
def driver_update_mode(con, schema_name, statecd, invyr, countycd, plot):
    ...
    classifier = Classifier(key_eastern_us.ClassificationKey())
    ...
    print(
        f"Run parameters: (schema_name={schema_name}, statecd={statecd}, invyr={invyr}, countycd={countycd!s}, plot={plot!s})"
    )
    # print("SELECT statement: " + ' '.join(sql.split()))
    print(
        "Updating " + schema_name + "." + "NIMS_COND_NVCS_TBL... ", end="", flush=True
    )
    ...
    print(
        f"{failure_count: 6d} forested conditions were assigned an NVCS code of {NID_ALGORITHM_FAILED:d} because the algorithm failed to produce a classification"
    )
    print(
        f"{ecoregion_not_supported_count:6d} forested conditions were assigned an NVCS code of {NID_ECOREGION_NOT_SUPPORTED:d} because the eco-subsection is not supported by the algorithm"
    )
    print(
        f"{skipped_count: 6d} conditions were skipped because the status is not forested"
    )
    print(" -----")
...
def driver_delete_mode(con, schema_name, statecd, invyr, countycd, plot):
    ...
    sql = (
        "DELETE FROM "
        + schema_name
        + ".nims_cond_nvcs_tbl WHERE cnd_cn IN ("
        + innersql
        + ")"
    )
    ...
    print(
        "Deleting from " + schema_name + ".NIMS_COND_NVCS_TBL... ", end="", flush=True
    )
...
def driver(connect_string, schema_name, mode, statecd, invyr, countycd=None, plot=None):
    ...
    print(
        "NVCS version " + __version__ + " on (" + time.asctime(time.localtime()) + ")"
    )
    with Timer() as t:
        ...
        with cx_Oracle.connect(connect_string) as con:
            if mode.upper() == "UPDATE":
                ...
                print(
                    mode.upper()
                    + " is not a valid mode.  The parameter value must be 'UPDATE' or 'DELETE'."
                )
            con.commit()
    print("{0:.03f} sec.\n".format(t.interval))
...
if __name__ == "__main__":
    ...
    print(
        "driver('my_username/my_password@LUPE01P', 'fs_nims_nrs', 'UPDATE', 20, 2009)"
    )
    print()
    print("2. Load 2009 Kansas, countycd=1 at LUPE:")
    print(
        "driver('my_username/my_password@LUPE01P', 'fs_nims_nrs', 'UPDATE', 20, 2009, 1)"
    )
    print()
    print("3. Load 2009 Kansas, countycd=1, plot=251 at LUPE:")
    print(
        "driver('my_username/my_password@LUPE01P', 'fs_nims_nrs', 'UPDATE', 20, 2009, 1, 251)"
    )
    print()
    print("4. Delete 2009 Kansas at LUPE:")
    print(
        "driver('my_username/my_password@LUPE01P', 'fs_nims_nrs', 'DELETE', 20, 2009)"
    )
    print()
